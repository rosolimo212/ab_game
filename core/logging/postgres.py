# coding: utf-8
"""Логирование в postgres (схема ab_game)."""
...
class PostgresLogger(EventLogger):
    """Запись пользователей и событий в postgres."""
        ...
        with postgres_connection(self.logging_config) as conn:
            with conn.cursor() as cur:
                ...
                if seq_row is None or not seq_row[0]:
                    raise RuntimeError(
                        "Не найден sequence для ab_game.users.internal_user_id"
                    )
    ...
    def log_event(
        ...
        params_json = (
            json.dumps(event_parameters, ensure_ascii=False)
